...
public class TradeLog {
	...
	*/
	public void tradeReceived(String itemName, int itemAmount) {
		...
		try {
			BufferedWriter bItem = new BufferedWriter(new FileWriter("./deps/data/tradesandothers/trades/received/" + c.playerName + ".txt", true));	//TODO deps stuff
			try {			
				bItem.newLine();
	...
	*/
	public void tradeGive(String itemName, int itemAmount) {
		...
		Calendar C = Calendar.getInstance();
		 try {
			BufferedWriter bItem = new BufferedWriter(new FileWriter("./deps/data/tradesandothers/trades/gave/" + c.playerName + ".txt", true));	//TODO deps stuff
			try {			
				bItem.newLine();
				bItem.write("Year : " + C.get(Calendar.YEAR) + "\tMonth : " + C.get(Calendar.MONTH) + "\tDay : " + C.get(Calendar.DAY_OF_MONTH));
				bItem.newLine();
