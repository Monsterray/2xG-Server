	...
	public void sendSmelting() {
		for (int j = 0; j < SMELT_FRAME.length; j++) {
			c.getPA().sendFrame246(SMELT_FRAME[j], 150, SMELT_BARS[j]); 
			        //sendFrame246(int interfaceFrameId, int SubFrame, int itemId);
		}
		c.getPA().sendFrame164(2400); //opens the interface in the chat area
		c.smeltInterface = true; //I think this is to make sure you are in the smithing interface
	}
