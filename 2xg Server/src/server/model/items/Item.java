...
public class Item {
	...
	static {
		...
		try {
			FileInputStream dataIn = new FileInputStream(new File("./deps/Data/data/stackable.dat"));
...
int[] stackableItems = {15243};
		...
		try {
			FileInputStream dataIn = new FileInputStream(new File("./deps/Data/data/notes.dat"));
			while ((c = dataIn.read()) != -1) {
				if (c == 0) {
		...
		counter = 0;
		try {
			FileInputStream dataIn = new FileInputStream(new File("./deps/Data/data/equipment.dat"));
			while ((c = dataIn.read()) != -1) {
