	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
			...
			c.setSidebarInterface(0, 776); //was 8460 //reap, chop, jab, block
			c.getPA().sendFrame246(777, 200, Weapon);
			c.getPA().sendFrame126("scythe", 779);
		} else if (c.playerEquipment[c.playerWeapon] == 10735) {
			c.setSidebarInterface(0, 776); //reap, chop, jab, block
			c.getPA().sendFrame246(777, 200, Weapon);
			c.getPA().sendFrame126("scythe", 779);	
	...
	**/
	...
	public void getRequirements(String itemName, int itemId) {
		...
		switch(itemId) {
			...
			case 15443:
			case 15444:
			case 4151: //Abyssal whip // if you don't want to use names 
				c.attackLevelReq = 70;
			return;
...
c.rangeLevelReq = 80;
	...
	**/
	...
	public boolean wearItem(int wearID, int slot) {
		System.out.println("dude he put somthing on");
		...
		synchronized(c) {
			...
			*/
				...
				if(slot >= 0 && wearID >= 0) {
					...
					* Castle wars by Monsterray
					*/
					if (CastleWars.SARA_BANNER == toRemove || CastleWars.ZAMMY_BANNER == toRemove) {  // alk update
			...
			if(c.getOutStream() != null && c != null) {
				if(c.playerEquipment[slot] > -1){
					...
					if(addItem(c.playerEquipment[slot], c.playerEquipmentN[slot])) {
						/*if (c.playerEquipment[slot] == CastleWars.SARA_CAPE || c.playerEquipment[slot] == CastleWars.ZAMMY_CAPE) {
                            c.sendMessage("Removed CastleWars Cape'test");
	...
	*/
	...
	public boolean bankWornEquipment(int itemID, int fromSlot, int amount){
		c.getPA().sendFrame126(bankSlotsUsed()+" / "+Config.BANK_SIZE, 23003);
		if(c.inTrade) {
			c.sendMessage("You can't store items while trading!");
			return false;
		...
		if (!Item.itemIsNote[c.playerEquipment[fromSlot] + 1]) {
			if (c.playerEquipment[fromSlot] <= 0) {
				return false;
			}
			if (Item.itemStackable[c.playerEquipment[fromSlot] + 1]) {
				int toBankSlot = 0;
				boolean alreadyInBank = false;
			    for (int i=0; i< Config.BANK_SIZE; i++) {
					if (c.bankItems[i] == c.playerEquipment[fromSlot]) {
						alreadyInBank = true;
						toBankSlot = i;
						i=Config.BANK_SIZE+1;
					}
				...
				if (!alreadyInBank && freeBankSlots() > 0) {
					...
					c.bankItems[toBankSlot] = c.playerEquipment[fromSlot] + 1;
					if ((c.bankItemsN[toBankSlot] + amount) <= Config.MAXITEM_AMOUNT && (c.bankItemsN[toBankSlot] + amount) > -1) {
						c.bankItemsN[toBankSlot] += amount;
					} else {
						c.sendMessage("Bank full!");
				...
				itemID = c.playerEquipment[fromSlot];
				int toBankSlot = 0;
				boolean alreadyInBank=false;
			    for (int i=0; i<Config.BANK_SIZE; i++) {
					if (c.bankItems[i] == c.playerEquipment[fromSlot]) {
				...
				if (!alreadyInBank && freeBankSlots() > 0) {
					...
					while (amount > 0) {
						itemExists = false;
						for (int i=firstPossibleSlot; i<c.playerEquipment.length; i++) {
								if ((c.playerEquipment[i]) == itemID) {
									firstPossibleSlot = i;
									itemExists = true;
									i=30;
						...
						if (itemExists) {
								c.bankItems[toBankSlot] = c.playerEquipment[firstPossibleSlot] + 1;
								c.bankItemsN[toBankSlot] += 1;
								amount--;
						} else {
								amount=0;
		...
		}else if(Item.itemIsNote[c.playerEquipment[fromSlot] + 1] && !Item.itemIsNote[c.playerEquipment[fromSlot]-2]){
			if (c.playerEquipment[fromSlot] <= 0) {
				return false;
			}
			if (Item.itemStackable[c.playerEquipment[fromSlot] + 1]) {
				...
				boolean alreadyInBank=false;
			    for (int i=0; i<Config.BANK_SIZE; i++) {
					if (c.bankItems[i] == (c.playerEquipment[fromSlot] + 1)) {
						alreadyInBank = true;
						toBankSlot = i;
						i=Config.BANK_SIZE+1;
					}
			...
			c.sendMessage("Item not supported "+(c.playerEquipment[fromSlot] + 1));
			return false;
		}
	}
